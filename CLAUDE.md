...
- The hero art is admin-switchable (`heroArt`): ascii cat `public/assets/kitokat-ascii-fine.avif` (default, rendered from `kitokat-raw.jpg`), dot-art hands `public/assets/hero-hands.*`, or an uploaded picture (`heroArtCustom`); partner logos in `public/assets/`.
- Fonts: Satoshi (Fontshare); the Cyrillic companion for RU is admin-selectable (`fontRu`: Onest default, Carlito, Jost - all loaded from Google Fonts). Every interface string can also override it per key via `i18nFontRu` (chips under the RU inputs in the admin Interface text page). Icons: simpleicons CDN.
