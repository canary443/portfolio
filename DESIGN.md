...
## Type
- **Satoshi** 400/500, self-hosted (`app/fonts.css`) — everything. Weight 400 headlines, hierarchy through size and tracking, never bold shouting.
...
- Media carousels (cards + modal): shadcn/embla with the fade plugin (`components/MediaCarousel.tsx`) - crossfading slides, drag/swipe on touch, SVG chevron arrows in the glass chips, dot indicators (clickable in the modal). Card photos auto-advance every 5s, video slides advance when the video ends; only the visible slide's video plays.
...
- **`prefers-reduced-motion: reduce`**: native scroll (no lenis), no parallax/curtain transforms, hero entrance becomes a plain fade, reveals show instantly, marquee stops, native cursor back, gif backgrounds show their still poster.
...
## Icons & imagery
- brand icons are inline SVG components in `components/BrandIcons.tsx` (Simple Icons paths, CC0), 12–18px, colored through `currentColor` or a `color` prop.
- Service icons: gold text glyphs (▤ ◉ ↻ &lt;/&gt;) or user-uploaded images from the admin.
- No photography except project screenshots; the hero art sits on pure black and is picked in the admin (`heroArt`): the ascii cat (`kitokat-ascii-fine.avif`, real monospace glyphs rendered from `kitokat-raw.jpg`, 80% width) - default, the braille cat (`kitokat-braille.avif`, 32% width), the dot-matrix hands (`hero-hands.*`, full-bleed 114%), or an uploaded picture (86% width, auto-rendered to ascii). `heroArtScale` is a percent multiplier on top of those widths.
- Behind the art, `heroBg` can also run a webgl preset or a `gif` mode: an uploaded gif / video at `heroBgGifOpacity`, swapped for a still poster when the visitor asks for less motion.
